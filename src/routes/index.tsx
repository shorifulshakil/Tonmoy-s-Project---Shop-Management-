  ...
  head: () => ({
    meta: [
      { title: "Zebra Outfit — Sales & Stock Management" },
      {
        name: "description",
        content:
          "Secure sales and inventory dashboard for Zebra Outfit clothing brand.",
      },
    ],
  }),
});
    ...
    <main className="min-h-screen flex items-center justify-center px-6 py-16">
      <div className="max-w-3xl w-full text-center">
        ...
        <p className="mt-8 text-muted-foreground max-w-xl mx-auto leading-relaxed">
          A private command center for the Zebra Outfit team — inventory, daily
          sales, discounts, and demand insights in one secure place.
        </p>
        <div className="mt-10 flex gap-3 justify-center flex-wrap">
          <Link to="/auth">
            <Button
              size="lg"
              className="bg-gold text-primary-foreground hover:opacity-90 px-8 rounded-sm font-bold tracking-luxe uppercase"
            >
          ...
          <Link to="/auth" search={{ mode: "signup" }}>
            <Button
              size="lg"
              variant="outline"
              className="border-gold/40 text-gold hover:bg-gold/10 rounded-sm font-bold tracking-luxe uppercase"
            >
              Create Account
            </Button>
          </Link>
          ...
          {[
            {
              ...
              text: "Daily sale %, weekly trends, category splits — at a glance.",
            },
            {
              icon: <Package className="w-5 h-5" />,
              title: "Stock Control",
              text: "Track sizes, low-stock alerts, and full SKU history.",
            },
            {
              icon: <Tag className="w-5 h-5" />,
              title: "Discounts & Demand",
              text: "Surface trending products and active discounts instantly.",
            },
          ].map((f) => (
            <div
              key={f.title}
              className="bg-card border border-border rounded-[4px] p-5 hover:border-gold transition"
            >
