...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const unitsChange =
      yestUnits > 0
        ? ((todayUnits - yestUnits) / yestUnits) * 100
        : todayUnits > 0
          ? 100
          : 0;
    ...
    sales.forEach((s) =>
      soldMap.set(s.product_id, (soldMap.get(s.product_id) ?? 0) + s.quantity),
    );
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="ml-[260px] min-h-screen">
        ...
        <div className="p-8">
          {section === "dashboard" && (
            <DashboardSection stats={stats} sales={sales} products={products} />
          )}
          {section === "products" && (
            <ProductsSection
              products={filtered}
          ...
          {section === "daily" && (
            <DailySection stats={stats} sales={sales} products={products} />
          )}
          {section === "stock" && (
            <StockSection stats={stats} products={products} />
          )}
        </div>
      </main>
...
function KpiCard({
  ...
  return (
    <div
      className={`bg-card border border-border ${border} border-t-[3px] rounded-[4px] p-6 relative overflow-hidden hover:border-gold transition-all`}
    >
      <div className="text-[0.7rem] tracking-luxe uppercase text-muted-foreground mb-2.5">
        {label}
      </div>
      ...
      {sub && (
        <div
          className={`text-xs mt-2 ${trend === "up" ? "text-success" : trend === "down" ? "text-destructive" : "text-muted-foreground"}`}
        >
          {trend === "up" ? "↑ " : trend === "down" ? "↓ " : ""}
          {sub}
        </div>
...
function ChartCard({
  ...
  return (
    <div className="bg-card border border-border rounded-[4px] p-6">
      <div className="text-[0.75rem] tracking-luxe uppercase text-muted-foreground mb-5">
        {title}
      </div>
      {children}
    </div>
...
function SectionHeader({ title, sub }: { title: string; sub?: string }) {
  return (
    <div className="mb-5">
      ...
      {sub && (
        <p className="text-[0.78rem] text-muted-foreground mt-0.5">{sub}</p>
      )}
    </div>
  );
}
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <KpiCard
          ...
          value={fmtBDT(stats.todayRevenue)}
          sub={`${stats.change.toFixed(1)}%`}
        ...
        <KpiCard
          ...
          sub={`${stats.unitsChange.toFixed(1)}%`}
          trend={stats.unitsChange >= 0 ? "up" : "down"}
          icon={<ShoppingBag />}
          accent="success"
        />
        <KpiCard
          ...
          <thead>
            <tr className="border-b border-border bg-white/[0.02]">
              {["Product", "Category", "Qty", "Amount", "Time"].map((h) => (
                <th
                  key={h}
                  className="text-left px-5 py-3.5 text-[0.68rem] tracking-luxe uppercase text-muted-foreground"
                >
                  {h}
                </th>
              ))}
              ...
              recent.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm font-mono text-gold">
                      {fmtBDT(Number(s.total))}
                    </td>
                    <td className="px-5 py-3.5 text-xs text-muted-foreground font-mono">
                      {new Date(s.sale_date).toLocaleString("en", {
                        hour: "2-digit",
                        minute: "2-digit",
                        month: "short",
                        day: "numeric",
    ...
    <>
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <Input
          ...
          className="flex-1 min-w-[200px] bg-card border-border"
        />
        {categories.map((c) => (
          <button
            key={c}
            onClick={() => setCatFilter(c)}
            className={`px-4 py-2.5 border rounded-sm text-[0.82rem] transition ${catFilter === c ? "border-gold text-gold" : "border-border text-muted-foreground hover:text-foreground"}`}
          >
            {c}
          </button>
        ))}
...
function DiscountedSection({
  ...
  return (
    <>
      <SectionHeader
        title="🏷️ Discounted Products"
        sub={`${products.length} items currently on sale`}
      />
      {products.length === 0 ? (
...
function TrendingSection({
  ...
  items: { p: Product; sold: number; rev: number }[];
}) {
  const max = Math.max(1, ...items.map((i) => i.sold));
  return (
    <>
      <SectionHeader
        title="🔥 Trending & High-Demand Products"
        sub="Ranked by units sold"
      />
      {items.length === 0 ? (
        <Empty
          icon="🔥"
          text="Record sales or feature products to populate this list."
        />
      ) : (
        <div className="flex flex-col gap-3">
          {items.map((it, i) => (
            <div
              key={it.p.id}
              className="bg-card border border-border rounded-[4px] px-5 py-4 flex items-center gap-4 hover:border-gold transition"
            >
              <div
                className={`font-display text-2xl font-black w-9 text-center shrink-0 ${i < 3 ? "text-gold" : "text-gold/25"}`}
              >
                #{i + 1}
              </div>
              ...
              <div className="w-28 hidden md:block">
                <div className="bg-white/[0.06] rounded-full h-1.5 overflow-hidden">
                  <div
                    className="h-full bg-gradient-bar rounded-full transition-all"
                    style={{ width: `${(it.sold / max) * 100}%` }}
                  />
                </div>
              </div>
...
function DailySection({
  ...
  const catRows = useMemo(() => {
    ...
    const total =
      Array.from(map.values()).reduce((a, v) => a + v.revenue, 0) || 1;
    return Array.from(map.entries())
      .map(([cat, v]) => ({ cat, ...v, share: (v.revenue / total) * 100 }))
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="bg-white/[0.06] rounded-full h-1.5 w-24 overflow-hidden">
                          <div
                            className="h-full bg-gradient-bar"
                            style={{ width: `${r.share}%` }}
                          />
                        </div>
    ...
    <>
      ...
      <div className="bg-card border border-border rounded-[4px] overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-white/[0.02]">
              {["Product", "Category", "Sizes", "Stock", "Status", "Price"].map(
                (h) => (
                  <th
                    key={h}
                    className="text-left px-5 py-3.5 text-[0.68rem] tracking-luxe uppercase text-muted-foreground"
              ...
              products.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`px-5 py-3.5 text-sm font-semibold ${status.c}`}
                    >
                      {status.l}
                    </td>
                    <td className="px-5 py-3.5 text-sm font-mono text-gold">
